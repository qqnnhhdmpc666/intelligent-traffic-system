...
async def run_comprehensive_test():
    ...
    async with TrafficSystemPerformanceTester() as tester:
        ...
        print("\n🧭 路径规划性能:")
        print(f"   总请求数: {path_result.total_requests}")
        ...
        print("\n🚗 交通数据更新性能:")
        print(f"   总请求数: {traffic_result.total_requests}")
        ...
        print("\n📊 论文评价指标 (对应4.2节):")
        print(f"   平均行程时间: {server_metrics['average_trip_time']:.1f}ms")
        print(f"   平均延误时间: {server_metrics['average_delay_time']:.1f}ms")
        print(f"   路网总吞吐量: {server_metrics['network_throughput']:.1f} 请求/秒")
        print(f"   路网平均速度: {server_metrics['average_network_speed']:.1f}/100")
        print(f"   拥堵指数: {server_metrics['congestion_index']:.1f}%")

        print("\n🔍 测试总结:")
        test_summary = server_metrics.get('test_summary', {})
